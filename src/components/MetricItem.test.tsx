...
describe("MetricItem", () => {
  it("renders metric label with index", () => {
    ...
    render(
      <Provider store={store}>
        <MetricItem item={createItem()} index={0} isSingleItem={false} />
      </Provider>,
    );
  ...
  it("does not render remove button when isSingleItem is true", () => {
    ...
    render(
      <Provider store={store}>
        <MetricItem item={createItem()} index={0} isSingleItem={true} />
      </Provider>,
    );
  ...
  it("renders multiple filters", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /remove filter/i }),
    ).toHaveLength(2);
  });
