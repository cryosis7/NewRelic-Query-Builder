...
describe("TimePicker", () => {
  ...
  it("does not call onChange when invalid time is entered", async () => {
    ...
    const timeInput = screen.getByDisplayValue("14:30");
    // Try to set an invalid time format
...
describe("DateTimeInput", () => {
  ...
  it("displays the provided time value", () => {
    ...
    expect(screen.getByLabelText("Time", { selector: "input" })).toHaveValue(
      "14:30",
    );
  });
...
describe("TimePeriodSelector", () => {
  ...
  it("renders time pickers for absolute mode", () => {
    ...
    expect(
      screen.getAllByLabelText("Time", { selector: "input" }),
    ).toHaveLength(2);
  });
  ...
  it("updates atom when relative input changes", () => {
    ...
    fireEvent.change(
      screen.getByLabelText("Relative", { selector: 'input[type="text"]' }),
      {
        target: { value: "3h ago" },
      },
    );
  ...
  it("allows typing custom relative values not in preset list", () => {
    ...
    const input = screen.getByLabelText("Relative", {
  ...
  it("updates dropdown when relative value changes from external source", () => {
    const store = createStore();
    store.set(timePeriodAtom, {
      mode: "relative",
      since: "2026-01-28T08:00",
    ...
    expect(input).toHaveValue("1h ago");

    // Update the atom from outside (simulates reset button behavior)
    store.set(timePeriodAtom, {
      mode: "relative",
      since: "2026-01-28T08:00",
      until: "2026-01-28T09:00",
    ...
    // The input should now show the updated value
    expect(input).toHaveValue("3h ago");

    // The dropdown component should have a new key, forcing it to re-render
    // This ensures defaultSelectedOptionId is re-evaluated
    const combobox = screen.getByRole("combobox", { name: /time period/i });
