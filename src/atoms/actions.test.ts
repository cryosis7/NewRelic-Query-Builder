...
describe("Action Atoms", () => {
  describe("resetAtom", () => {
    it("resets all atoms to initial state values", () => {
      ...
      expect(store.get(excludeHealthChecksAtom)).toBe(
        initial.excludeHealthChecks,
      );
      expect(store.get(excludeBulkEndpointAtom)).toBe(
        initial.excludeBulkEndpoint,
      );
  ...
  describe("saveQueryAtom", () => {
    it("saves a query with the correct name and current state", () => {
      const store = createStore();

      store.set(saveQueryAtom, { name: "My Query" });

      const saved = store.get(savedQueriesAtom);
      expect(saved).toHaveLength(1);
      expect(saved[0].name).toBe("My Query");
      expect(saved[0].id).toBeDefined();
      expect(saved[0].createdAt).toBeDefined();
      expect(saved[0].nrqlQuery).toContain("SELECT");
      expect(saved[0].state.applications).toEqual(["global-tax-mapper-api"]);
      expect(saved[0].state.environment).toBe("prod");
    });

    it("trims whitespace from the name", () => {
      const store = createStore();

      store.set(saveQueryAtom, { name: "  Spaced Name  " });

      const saved = store.get(savedQueriesAtom);
      expect(saved[0].name).toBe("Spaced Name");
    });

    it("captures the current state snapshot", () => {
      const store = createStore();

      // Change state before saving
      store.set(environmentAtom, "uat");
      store.set(applicationsAtom, ["global-tax-mapper-bff"]);

      store.set(saveQueryAtom, { name: "UAT BFF Query" });

      const saved = store.get(savedQueriesAtom);
      expect(saved[0].state.environment).toBe("uat");
      expect(saved[0].state.applications).toEqual(["global-tax-mapper-bff"]);
      expect(saved[0].nrqlQuery).toContain("global-tax-mapper-bff-uat");
    });

    it("appends multiple saved queries", () => {
      const store = createStore();

      store.set(saveQueryAtom, { name: "First" });
      store.set(saveQueryAtom, { name: "Second" });
    ...
    it("generates unique IDs for each saved query", () => {
      const store = createStore();

      store.set(saveQueryAtom, { name: "A" });
      store.set(saveQueryAtom, { name: "B" });
  ...
  describe("deleteSavedQueryAtom", () => {
    it("removes a saved query by ID", () => {
      const store = createStore();

      store.set(saveQueryAtom, { name: "To Keep" });
      store.set(saveQueryAtom, { name: "To Delete" });

      const saved = store.get(savedQueriesAtom);
      expect(saved).toHaveLength(2);

      store.set(deleteSavedQueryAtom, saved[1].id);

      const remaining = store.get(savedQueriesAtom);
    ...
    it("does nothing if ID does not exist", () => {
      ...
      store.set(saveQueryAtom, { name: "Existing" });
      store.set(deleteSavedQueryAtom, "nonexistent-id");
    ...
    it("can delete all saved queries one by one", () => {
      const store = createStore();
      ...
      const saved = store.get(savedQueriesAtom);
      store.set(deleteSavedQueryAtom, saved[0].id);
      store.set(deleteSavedQueryAtom, saved[1].id);
