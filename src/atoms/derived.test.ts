...
describe("Time Period Derived Atoms", () => {
  describe("sinceDateAtom", () => {
    ...
    it("defaults time to 00:00 when parseDateTime returns empty time", () => {
      const store = createStore();
      // Mock parseDateTime to return empty time to trigger the || '00:00' branch
      const parseSpy = vi
        .spyOn(dateTimeUtils, "parseDateTime")
        .mockReturnValue({
          date: "2026-01-28",
          time: "",
        });
  ...
  describe("untilDateAtom", () => {
    ...
    it("defaults time to 23:59 when parseDateTime returns empty time", () => {
      const store = createStore();
      const parseSpy = vi
        .spyOn(dateTimeUtils, "parseDateTime")
        .mockReturnValue({
      ...
      expect(store.get(timePeriodAtom).until).toBe("2026-06-20T23:59");
