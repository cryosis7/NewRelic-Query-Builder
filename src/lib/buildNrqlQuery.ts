...
function normalizeAggregationForMetric(
  field: string,
  aggregationType: AggregationType,
): AggregationType {
  const fieldDef = getFieldByName(field);
...
function parseRelativeTimeInput(value: string): string | null {
  ...
  const match = normalized.match(
    /^([0-9]+)\s*(m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days)\s*(ago)?$/i,
  );
  if (!match) {
    return null;
  }
...
export function buildNrqlQuery(state: QueryState): string {
  ...
  const itemFilters = state.metricItems.map((item) =>
    buildFilterConditions(item.filters),
  );

  const conditionKey = (conditions: string[]) =>
  ...
  if (state.timePeriod.mode === "relative") {
    ...
    const sinceValue =
      state.timePeriod.since ||
      new Date(Date.now() - 3600000).toISOString().slice(0, 16);
    const untilValue =
      state.timePeriod.until || new Date().toISOString().slice(0, 16);
