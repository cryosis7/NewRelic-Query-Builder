...
export const AGGREGATION_TYPES: AggregationConfig[] = [
  {
    value: "average",
    label: "Average",
    nrqlTemplate: "average({field})",
    isNumericalAggregator: true,
  },
  { value: "count", label: "Count", nrqlTemplate: "count({field})" },
  {
    value: "p95",
    label: "95th Percentile",
    nrqlTemplate: "percentile({field}, 95)",
    isNumericalAggregator: true,
  },
...
export type StringOperator = "=" | "IN" | "LIKE";
...
export const NRQL_FIELDS: NrqlField[] = [
  ...
  createNrqlField("appName", "Application", "string", {
    canSearch: false,
    canFilter: false,
  }),
];
...
export const SEARCH_FIELDS = NRQL_FIELDS.filter((f) => f.canSearch).map(
  (f) => ({
    value: f.name,
    label: f.label,
  }),
);
export const FILTER_FIELDS = NRQL_FIELDS.map((f) => ({
  value: f.name,
  label: f.label,
}));
export const FACET_OPTIONS: { value: FacetOption; label: string }[] = [
  { value: "none", label: "No Facet" },
  ...NRQL_FIELDS.filter((f) => f.canFacet).map((f) => ({
    value: f.name,
    label: f.label,
  })),
];
...
export function getOperatorsForField(
  fieldName: string,
): { value: MetricFilterOperator; label: string }[] {
