...
test.describe('Metric Filters', () => {
  ...
  test.describe('Duration Filters (5.2)', () => {
    ...
    test('5.2.2 Change operator - Change to >=, verify query updates', async ({ page }) => {
      ...
      const operatorDropdown = page.getByRole('combobox').filter({ hasText: '>' }).first();
  ...
  test.describe('Response Status Filters (5.3)', () => {
    test('5.3.1 Select response.status field - Change filter field', async ({ page }) => {
      ...
      // Find field dropdown - it's the last combobox that contains "Duration" text (after metric type and aggregation)
      const fieldDropdown = page.getByRole('combobox').filter({ hasText: 'Duration' }).last();
