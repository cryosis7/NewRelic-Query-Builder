...
test.describe("Application Selection", () => {
  ...
  test("Default state - API checkbox is checked by default", async ({
    page,
  }) => {
  ...
  test("Select single app - Check BFF updates query", async ({ page }) => {
    // Check BFF
    const bffCheckbox = page.getByRole("checkbox", { name: "BFF" });
  ...
  test("Select multiple apps - All three apps in query", async ({ page }) => {
  ...
  test("Deselect all apps - Shows error message", async ({ page }) => {
  ...
  test("Toggle app selection - Check and uncheck Integrator API", async ({
    page,
  }) => {
    const queryPreview = getQueryPreview(page);
