...
test.describe('Query Preview', () => {
  ...
  test.describe('Query Display (9)', () => {
    test('9.1 Query displayed - Generated NRQL visible in preview', async ({ page }) => {
    ...
    test('9.5 Copy button disabled with invalid query (no apps)', async ({ page }) => {
  ...
  test.describe('Query Structure Validation (10)', () => {
    test('10.1 Basic query structure - Default state produces valid NRQL', async ({ page }) => {
      const queryPreview = getQueryPreview(page);
      const queryText = await queryPreview.textContent();
    ...
    test('10.5 App-environment combination - BFF + UAT produces correct app name', async ({ page }) => {
      // Select BFF only
      await page.getByRole('checkbox', { name: 'API', exact: true }).uncheck();
  ...
  test.describe('Edge Cases & Error Handling (11)', () => {
    test('11.1 No applications - Shows error message', async ({ page }) => {
      // Uncheck all apps
      await page.getByRole('checkbox', { name: 'API', exact: true }).uncheck();
