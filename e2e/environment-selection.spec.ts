...
test.describe("Environment Selection", () => {
  ...
  test("Default environment - Production is selected with -prod suffix", async ({
    page,
  }) => {
  ...
  test("Switch to UAT - Query updates to -uat suffix", async ({ page }) => {
  ...
  test("Switch back to Prod - Query returns to -prod suffix", async ({
    page,
  }) => {
    // First switch to UAT
    const envDropdown = page.getByRole("combobox", { name: "Environment" });
  ...
  test("Multiple apps - All app names update suffix", async ({ page }) => {
    // Enable multiple apps first
    const bffCheckbox = page.getByRole("checkbox", { name: "BFF" });
    const integratorCheckbox = page.getByRole("checkbox", {
      name: "Integrator API",
    });
    ...
    await expect(queryPreview).toContainText(
      "global-tax-mapper-integrator-api-prod",
    );
    ...
    await expect(queryPreview).toContainText(
      "global-tax-mapper-integrator-api-uat",
    );
  });
});
