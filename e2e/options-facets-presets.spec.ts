...
test.describe("Options, Facets, and Presets", () => {
  ...
  test.describe("Options (6)", () => {
    ...
    test("Disable health check exclusion - Filter removed from query", async ({
      page,
    }) => {
    ...
    test("Default timeseries - Use TIMESERIES is checked", async ({ page }) => {
    ...
    test("Disable timeseries - TIMESERIES clause removed", async ({ page }) => {
      // Uncheck timeseries
      const timeseriesCheckbox = page.getByRole("checkbox", {
        name: "As Timeseries",
      });
  ...
  test.describe("Facet Selection (7)", () => {
    ...
    test("Change to Response Status - Query updates", async ({ page }) => {
      // Click facet dropdown and select Response Status
      const facetDropdown = page.getByRole("combobox", { name: "Facet By" });
      await facetDropdown.click();
      await page
        .getByRole("option", { name: "Response Status" })
    ...
    test("Change to Request Method - Query updates", async ({ page }) => {
      // Click facet dropdown and select Request Method
      const facetDropdown = page.getByRole("combobox", { name: "Facet By" });
      await facetDropdown.click();
      await page
        .getByRole("option", { name: "Request Method" })
        .waitFor({ state: "visible" });
    ...
    test("Change to Name - Query updates", async ({ page }) => {
    ...
    test("Disable facet - Select No Facet, clause removed", async ({
      page,
    }) => {
      ...
      await facetDropdown.click();
      await page
        .getByRole("option", { name: "No Facet" })
        .waitFor({ state: "visible" });
  ...
  test.describe("Common Queries/Presets (8)", () => {
    test("Preset panel visible - Shows 3 preset buttons and Reset", async ({
      ...
      await expect(
        page.getByRole("heading", { name: "Common Queries" }),
      ).toBeVisible();

      // All 3 preset buttons + Reset should be visible
      await expect(
        page.getByRole("button", { name: "API Throughput - Last 3 Hours" }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: "API Latency - Last 3 Hours" }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: "API Error Count" }),
      ).toBeVisible();
      await expect(page.getByRole("button", { name: "Reset" })).toBeVisible();
    });

    test("API Throughput - Last 3 Hours - Click updates form", async ({
      page,
    }) => {
      // Click the preset
      await page
        .getByRole("button", { name: "API Throughput - Last 3 Hours" })
        .click();
      ...
      await expect(
        page.getByRole("checkbox", { name: "BFF" }),
      ).not.toBeChecked();
      await expect(
    ...
    test("API Latency - Last 3 Hours - Click updates form", async ({
      page,
    }) => {
      // Click the preset
      await page
        .getByRole("button", { name: "API Latency - Last 3 Hours" })
        .click();
    ...
    test("API Error Count - Click updates form", async ({ page }) => {
      // Click the preset
